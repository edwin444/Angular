...
@Component({
  selector: 'pm-inner-modal',
  templateUrl: './inner-modal.component.html',
  styleUrls: ['./inner-modal.component.css']
})
export class InnerModalComponent implements OnInit, AfterViewInit {

  //next we define a input from the calling modify button of the main table
  @Input() valuesArray;
  ...
  ngOnInit() {
    //got the existing values , now we will fill up the rows.
    console.log(this.valuesArray);
    for (var i of this.valuesArray)
    {
      //i is string 
      //console.log(i);
      this.rowData.push({value: i});
    }
  }
  ngAfterViewInit(){
  }
  ...
  onBtnClick1(e) {
    ...
    //   //check which displayed row is same as the passed rowNode
    //   if (RowNode == nodeClicked)
    //   {
    //     //then we insert a row inline.
    //     this.addItemsAtIndex(i+1);
    //     console.log(`${i}+"  "+${nodeClicked.childIndex}`);
    //     break;
    //   }
    // }
  }
  ...
  onBtnClick2(e) {  
    ...
    //to get the value of the nodes in a json object
    // let tempArr = []
    // this.gridApi.forEachNodeAfterFilterAndSort((node, index)=>{
    //   tempArr.push(node.getValue("value"));
    // });

    //console.log("H: "+tempArr);
